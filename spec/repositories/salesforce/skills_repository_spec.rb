...
RSpec.describe Salesforce::SkillsRepository do
  ...
  context "skill wasn't synced before" do
    ...
    it "creates new entry in Salesforce" do
      set_salesforce_expectation(:create, "sf_id", Name: "Debugging", Description__c: "Squashing bugs", RateType__c: 'range')
    ...
    it "raises error if couldn't be created" do
      set_salesforce_expectation(:create, false, Name: "Debugging", Description__c: "Squashing bugs", RateType__c: 'range')
      expect do
        repository.sync(skill)
  ...
  context "skill was synced before" do
    ...
    it "updates old entry in Salesforce" do
      set_salesforce_expectation(:update, true, Id: "sf_id", Name: "Debugging", Description__c: "Squashing bugs", RateType__c: 'range')
      repository.sync(skill)
    end

    it "raises error if couldn't be updated" do
      set_salesforce_expectation(:update, false, Id: "sf_id", Name: "Debugging", Description__c: "Squashing bugs", RateType__c: 'range')
      expect do
        repository.sync(skill)
      end.to raise_error Salesforce::SkillsRepository::SyncFailed, /id=42/
    end
