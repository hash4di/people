...
describe Membership do
  subject { build(:membership, starts_at: DateTime.now) }
  ...
  describe '#validate_duplicate_project' do
    ...
    before do
      [
        [DateTime.new(2013, 1, 1), DateTime.new(2013, 6, 1)],
        [DateTime.new(2013, 6, 2), DateTime.new(2013, 8, 30)],
        [DateTime.new(2013, 10, 1), nil]
      ].each { |time_range| create(:membership, user: user, project: project, starts_at: time_range[0], ends_at: time_range[1]) }
    end

    context 'valid' do
      [
        [DateTime.new(2012, 1, 1), DateTime.new(2012, 6, 1)],
        [DateTime.new(2013, 9, 1), DateTime.new(2013, 9, 30)]
      ].each do |time_range|
    ...
    context 'invalid' do
      [
        [DateTime.new(2012, 1, 1), nil],
        [DateTime.new(2013, 11, 1), nil],
        [DateTime.new(2013, 1, 1), DateTime.new(2013, 5, 1)]
      ].each do |time_range|
        it "start #{time_range[0]} ends #{time_range[1]}" do
          membership.starts_at, membership.ends_at = time_range
  ...
  context 'notifies on slack if created' do
    describe '#notify_slack_on_create' do
      ...
      it 'notifies with proper message if only starts_at set' do
        ...
        expected_notification += " has been added to *#{membership.project.name}* since _#{membership.starts_at.to_date}_."
      ...
      it 'notifies with proper message of starts_at and ends_at set' do
        ...
        expected_notification += " _#{membership_with_ends_at.starts_at.to_date}_ to _#{membership_with_ends_at.ends_at.to_date}_."
